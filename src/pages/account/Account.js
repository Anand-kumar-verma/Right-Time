...
import { fron_end_main_domain, newdomain } from "../../services/urls";
...
function Account() {
  ...
  async function sendUrlCallBackToBackend(transactionId) {
    try {
      console.log("function",transactionId)
      const order= {
        order_id: transactionId
      }
      const res = await apiConnectorGet(
        `${newdomain}/api/v1/deposit-collback?order_id=${transactionId}`
      );
      if (res?.data?.msg === "Call back successfully.") {
  ...
  return (
    <Layout header={false}>
      <Container sx={style.container}>
            ...
            <Box className="flex justify-start items-center gap-2 ">
              ...
              <Box className="flex flex-col ">
                <Box className="flex justify-start items-center mt-5">
                  <Typography sx={{ color: bgtan, fontWeight: "700" }}>
                    {wallet?.full_name}
                  </Typography>
                  <Typography>
          ...
          >
            ...
            >
              ...
              >
                ...
                ₹ {Number(
                  Number(wallet?.wallet || 0) + Number(wallet?.winning || 0) + Number(wallet?.working_wallet || 0)
                )?.toFixed(2)}
          ...
          >
            ...
            >
              <Box className="flex items-center gap-2">
                ...
                <Typography className="!mt-1 text-white">
                  Game Notification
                </Typography>
              </Box>
              <IconButton>
        ...
        <Box
          sx={{ background: bglightgray, width: "95%", ml: "2.5%" }}
          className=" shadow-xl rounded-lg !mt-4 py-5"
        >
          <Typography className=" px-3" sx={{ color: "white" }}>
            Service Center
          </Typography>
          <Box className="grid grid-cols-3 m-5 justify-center gap-5">
            ...
            >
              ...
              <Typography className=" !text-white  !text-sm ml-2">
                {" "}
                Notification
              </Typography>
            </Box>
