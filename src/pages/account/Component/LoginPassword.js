...
function LoginPassword() {
  ...
  const { data: profile } = useQuery(["myprofile"], () => MyProfileDataFn(), {
  ...
  const fk = useFormik({
    ...
    onSubmit: () => {
      console.log(fk.values.old_pass, result?.password);
      if (
        !fk.values.old_pass ||
        !fk.values.new_pass ||
  ...
  const changePassword = async (reqbody) => {
    ...
    try {
      const response = await apiConnectorPost(endpoint.forget_password, reqbody, {
        headers: {
      ...
      <Container sx={style.container}>
        ...
        >
          <Box mt={2} component="form" onSubmit={fk.handleSubmit}>
            ...
            {isLoading && <CustomCircularProgress isLoading={isLoading} />}
          </Box>
        </Box>
      </Container>
    </Layout>
