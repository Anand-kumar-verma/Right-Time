...
function Withdrawl() {
  ...
  const withdraw_payment_Function = async (reqbody) => {
    setloding(true);
    try {
      const response = await apiConnectorPost(`${endpoint.withdraw_payment}`, reqbody);
      walletamountFn();
      if (response?.data?.msg === "Your Withdrawal Request will be approve by admin within 24 Hrs. Thank You!") {
        fk.handleReset();
