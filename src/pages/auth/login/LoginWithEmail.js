...
function LoginWithEmail() {
  ...
  const loginFunction = async (reqbody) => {
    setloding(true);
    try {
      const response = await apiConnectorPost(endpoint.login, reqbody, {
        headers: {
      ...
      ?navigate("/dashboard"): 
     (["Admin","SuperAdmin"]?.includes(response?.data?.user_type)) && navigate("/admindashboard");
