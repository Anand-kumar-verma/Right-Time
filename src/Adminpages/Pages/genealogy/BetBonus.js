...
import { getbetBonus } from "../../Services";
...
const BetBonus = () => {
  ...
  const { isLoading, data: Bet_bonus } = useQuery(
    ["bet_bonus_data" , value],
    () => getbetBonus({username:`${search}`}),
    {
      refetchOnMount: false,
      refetchOnReconnect: true,
    }
  );
  const player_new_data = Bet_bonus?.data?.data || [];
