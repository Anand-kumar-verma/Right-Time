...
import { getlevelBonus } from "../../Services";
...
const LevelBonus = () => {
  ...
  const { isLoading, data: Level_bonus } = useQuery(
    ["level_bonus_data" , value],
    () => getlevelBonus({ username:`${search}`}),
    {
      refetchOnMount: false,
      refetchOnReconnect: true,
    }
  );
  const player_new_data = Level_bonus?.data?.data || [];
