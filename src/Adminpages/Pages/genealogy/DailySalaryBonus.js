...
import { getDailySalaryBonus } from "../../Services";
import CustomTable from "../../Shared/CustomTable";
import moment from "moment";

const DailySalaryBonus = () => {
  ...
  const { isLoading, data: DailySalary_bonus } = useQuery(
    ["daily_salary_bonus_data" , value],
    () => getDailySalaryBonus({ username:`${search}`}),
    {
      refetchOnMount: false,
      refetchOnReconnect: true,
    }
  );
  const player_new_data = DailySalary_bonus?.data?.data || [];
